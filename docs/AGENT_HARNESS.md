...
**The fallback action is not neutral. This is a known defect.** A player may end its turn at any time after the roll, and may roll when the roll is owed, but the engine lists builds, maritime trades, and development-card plays before `end-turn`, and it lists development cards before both the roll and the robber move. The seed 47 series lost 90 board decisions to the fallback. In 41 of them a neutral action existed and the fallback spent something instead: 18 road builds, 11 maritime trades, 7 development-card purchases, 2 city builds, 2 knights played before the roll, and 1 knight played in the robber phase. In 31 it took the neutral action: 17 turn ends and 14 rolls. In the remaining 18 the rules leave no neutral action, because the setup placement, the discard, and the robber move are forced; every replaced setup placement chose `v:-1:-5`, the first vertex in the engine's order, whatever the board looked like. Game `terra-max-seed47-b` lost 23 of the 90 by itself, and its 3 discards came from one `turn.discard` phase that required three cards.
