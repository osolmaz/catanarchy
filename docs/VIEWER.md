...
The first release supports the regular three-player and four-player base-game board defined in the [engine design](ENGINE.md). The web viewer uses the Colonist terrain, number, shore, dock, port, robber, road, settlement, and city art listed in [Third-party assets](../THIRD_PARTY_ASSETS.md). The standard topology selects the correct shore and dock orientation for each board position. The engine and agent protocol do not depend on viewer art.
